	...
	switch(options.type){
		case 'object':
			...
			for(var requestObjectAttribute in requestArgument){
				if (requestObjectAttribute === "$") continue;
				var requestAttribute = requestArgument[requestObjectAttribute][0];

				if (typeof requestAttribute === "object") {
					requestAttribute = requestAttribute._;
				}
...
SoapServer.prototype.handleOperationRequest = function(service, soapRequest){
	const envelopeKey = Object.keys(soapRequest).find(k => k.endsWith('Envelope'));
	const envelope = soapRequest[envelopeKey];
	const bodyKey = Object.keys(envelope).find(k => k.endsWith('Body'));
	const body = envelope[bodyKey];
	var operationRequest = body[0];
	...
	for(var operationRequestName in operationRequest){
		...
		if (requestArguments.$) delete requestArguments.$;
		...
		for(var inputName in operation.inputs) {
			...
			var requestArgument = requestArguments[Object.keys(requestArguments).find(k => k.endsWith(inputName))];
			if('undefined' == requestArgument){
				if(input.required)
			...
			var requestArgument = this.castArgument(inputName, requestArgument, input);
...
SoapServer.prototype.getWsdl = function(host, service){
	...
	for(operationName in service.operations){
		...
		wsdl += '			<input name="' + operationName + 'Request">\n';
